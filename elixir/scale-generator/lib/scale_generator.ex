defmodule ScaleGenerator do
  ...
  def chromatic_scale(tonic \\ "C"), do: generate_scale(@sharp_scale, tonic)
  ...
  def flat_chromatic_scale(tonic \\ "C"), do: generate_scale(@flat_scale, tonic)
  ...
  def scale(tonic, pattern) do
    ...
    pattern
    |> String.codepoints()
    |> Enum.reduce({0, []}, &add_chromatic_scale(chromatic_scale, &1, &2))
    |> elem(1)
  ...
  defp add_chromatic_scale(chromatic_scale, interval, {last_index, acc}) do
    ...
    last_index = last_index + @semitone_steps[interval]
    {last_index, acc}
  end
end
