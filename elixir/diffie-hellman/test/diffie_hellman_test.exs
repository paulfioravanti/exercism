defmodule DiffieHellmanTest do
  ...
  test "private key should be between 1 and P-1, inclusive" do
    ...
    assert DiffieHellman.generate_private_key(prime_p) in Range.new(
             1,
             prime_p - 1
           )
  end
  ...
  test "public key correctly calculated from two primes and private key" do
    ...
    assert DiffieHellman.generate_public_key(prime_p, prime_g, private_key) ==
             expected_public_key
  end
  ...
  test "shared secret key correctly calculated from initial prime, Bob's public key, and Alice's private key" do
    ...
    assert DiffieHellman.generate_shared_secret(
             prime_p,
             bob_public_key,
             alice_private_key
           ) ==
  ...
  test "exchanging public keys between Alice and Bob should calculate the same shared secret" do
    ...
    alice_public_key =
      DiffieHellman.generate_public_key(prime_p, prime_g, alice_private_key)

    bob_public_key =
      DiffieHellman.generate_public_key(prime_p, prime_g, bob_private_key)
